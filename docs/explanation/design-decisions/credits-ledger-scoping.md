...
| T11 | Policy read endpoints | 1d | Done | `GET /accounting/prices` serves credit rates from the policy in force, brought forward as part of removing fiat pricing: `price` became `credits_per_unit`, and `uuid` and `valid_until` are gone. `eodhp-workspace-ui` reads this through `InvoicesContext` and needs updating to match. `GET /accounting/pricing-policy` serves the whole rate card in force as one version - every rate, every multiplier and the default category - and both endpoints take `at`, a hub admin's parameter, to read what priced usage at any other instant rather than what prices it now. It resolves by the same rule that priced the usage (D10), so an instant before every policy answers from the earliest rather than 404ing. **The version list is still not served**: asking about a date is a question with one answer, where paging the history is the audit read that `billing-admin ls <sku>` covers. The endpoint requires a token but asks nothing of its claims, through a `require_token` dependency now carried by every endpoint that has no workspace or account in its path; `at` is the one parameter this service restricts to `hub_admin` rather than to a workspace tier, because what prices usage now is a product read and what priced it in January is not |
| T12 | Usage reads with period, user and SKU filters | 1d | Done | Both `usage-data` endpoints report `credits` beside `quantity`, take `?user=` and `?sku=` filters, and take `?group-by=` to choose which of user, SKU and workspace each total is broken down by. Filters apply before aggregation, because the column a filter tests may be NULL after it. `group-by` defaults to `sku,workspace`, which is the shape the endpoint already had, and the empty set totals over the period alone. A dimension left out is reported as null, so `item` and `workspace` are nullable in the response where they used to be required - that is the cost of making grouping selectable, and `eodhp-workspace-ui` should be told. `group-by` without `time-aggregation` is a 422 rather than a no-op. Two departures from this note's plan, below |
...
Waves 0 to 5 total about 23 days. Done: T1 to T5, T7 to T13 and T22, which is 12.5 days, plus most of T6. This excludes the two tasks below that remain blocked, and matches the earlier estimate closely enough that the [ADR](accounting-billing-backend-adr.md) does not need revising.
