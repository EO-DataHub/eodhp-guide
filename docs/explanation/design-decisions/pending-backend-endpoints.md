...
| GET/PUT | `/api/workspaces/:id/accounting/storage-plan`                                | `{ planId: 'free' \| 'standard' \| 'plus' }`                                     | Reads/sets the workspace's storage plan tier and current storage usage in GB, for the Budget page's storage section — billed in pounds, separate from the credit endpoints above                                  | Owner            |
