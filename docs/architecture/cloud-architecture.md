...
Three clusters are used based on position in the deployment pipeline: test, staging and production. The test cluster is owned by developers and can receive both work-in-progress artefacts promoted through logical Kargo dev stages and release candidates that have passed peer review. After verification in test, the development team decides when to promote changes to staging for customer review. After customer agreement, they are promoted to the production cluster.
