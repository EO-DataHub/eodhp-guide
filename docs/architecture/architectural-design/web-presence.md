...
Many pieces of the web presence consist of UIs provided by other components, such as JupyterHub as described in 3.7 Jupyter, and we reuse existing interfaces as much as possible. In order to provide navigation, tie components into a cohesive site, provide static content and serve as a launch-point for client-side apps, an instance of the Wagtail CMS has been installed. 
...
The Catalogue Browser UI follows a similar pattern, although its location is not versioned and it does not use the CMS-rendered header and footer: it is deployed to an S3 location, served from the EODH domain name by CloudFront and referenced by the CMS. 
