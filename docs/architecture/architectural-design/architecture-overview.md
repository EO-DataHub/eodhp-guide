...
1. **Application End Users** - Users who use services created on top of EODHP by EODHP users, without necessarily knowing about EODHP itself.
...
**Data Streams** are collections of data and metadata provided to EODH users by data providers via an EODH integration. EODH facilities to find, visualize and use data are then available for those data streams. A data provider does not necessarily know they are an EODH data stream – for example, if EODH harvests a publicly available STAC Catalog – but typically will be collaborating in some form. 
